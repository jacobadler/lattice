 ...
 * C4 = 60, A4 = 69, etc.
 *
 * When useOutputOctave is true, the octave is interpreted as the desired
 * output pitch octave: raising the octave makes the keyboard sound higher.
 * Internally, this inverts the octave around 4 so that the tuning table
 * mapping produces the expected result (default octave 4 is unchanged).
 */
export function noteNameToMidi(name: NoteName, octave: number, useOutputOctave = false): number {
  const index = NOTE_NAMES.indexOf(name);
  const effectiveOctave = useOutputOctave ? 8 - octave : octave;
  return (effectiveOctave + 1) * 12 + index;
}
