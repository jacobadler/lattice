...
function App() {
  ...
  const [timbre, setTimbre] = useState<Timbre>('reed');
  ...
  useEffect(() => {
    const baseMidi = noteNameToMidi(midiRootNote, midiRootOctave, true);
  ...
  const getRatioKeyForMidiNote = useCallback((midiNote: number): string => {
    ...
    const rootMidi = noteNameToMidi(midiRootNoteRef.current, midiRootOctaveRef.current, true);
